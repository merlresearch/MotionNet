...
from data.data_utils import gen_2d_grid_gt, voxelize_occupy
...
parser.add_argument(
    "-r",
    "--root",
    default="/media/pwu/Data/3D_data/nuscene/all_nuscene",
    type=str,
...
parser.add_argument(
    "-p",
    "--savepath",
    default="/media/pwu/62316788-a8e6-423c-9ed3-303ebb3ab2de/pwu/temporal_data/train",
    type=str,
...
if args.split == "train":
    ...
    nsweeps_back = (
        25  # Setting this to 30 (for training) or 25 (for testing) allows conducting ablation studies on frame numbers
    )
    nsweeps_forward = 20
    skip_frame = 0
    num_adj_seqs = 1
...
def gen_data():
    ...
    for scene_idx in res_scenes:
        ...
        while curr_sample_data["next"] != "":
            ...
            all_pc, all_times, trans_matrices = LidarPointCloud.from_file_multisweep_bf_sample_data(
                nusc,
                curr_sample_data,
            ...
            for ann_token in corresponding_sample_rec["anns"]:
                ...
                instance_boxes, instance_all_times, _, _ = LidarPointCloud.get_instance_boxes_multisweep_sample_data(
                    nusc, curr_sample_data, instance_token, nsweeps_back=nsweeps_back, nsweeps_forward=nsweeps_forward
                )
            ...
            if adj_seq_cnt == num_adj_seqs:
                ...
                if num_instance_token_list > 1:
                    ...
                    for l in range(num_instance_token_list):
                        ...
                        assert (
                            counter == curr_save_data_dict["num_instances"]
                        ), "The number of instances is inconsistent."
...
def convert_to_dense_bev(data_dict):
    ...
    past_pcs_idx = past_pcs_idx[
        -num_past_frames_for_bev_seq:
    ]  # we typically use 5 past frames (including the current one)
    for i in past_pcs_idx:
        res, voxel_indices = voxelize_occupy(
            pc_list[i], voxel_size=voxel_size, extents=area_extents, return_indices=True
        )
...
def convert_to_sparse_bev(dense_bev_data):
    (
        ...
        save_voxel_points,
        save_pixel_indices,
        save_pixel_instance_maps,
        save_disp_field_gt,
        save_valid_pixel_maps,
        save_non_empty_maps,
        save_pixel_cat_maps,
        save_num_past_pcs,
        save_num_future_pcs,
